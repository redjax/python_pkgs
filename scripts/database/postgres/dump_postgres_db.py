...
Description:
    Wraps the `pg_dump` command to dump a PostgreSQL database to a SQL file. Includes a CLI, run with `--help` to see usage.
    
    **NOTE**: You must have a version of `pg_dump` installed. This can be installed with the `postgresql-client-xx` package, where
    `xx` is the version of PostgreSQL your database server is running. The server and `pg_dump` versions must match.
    
    Debian sometimes does not have the current version of `pg_dump` in their repositories. You can add it with:
    
    sudo apt install curl gnupg lsb-release -y
    curl -fsSL https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo gpg --dearmor -o /usr/share/keyrings/postgresql.gpg
    echo "deb [signed-by=/usr/share/keyrings/postgresql.gpg] http://apt.postgresql.org/pub/repos/apt $(lsb_release -cs)-pgdg main" | \\\nsudo tee /etc/apt/sources.list.d/pgdg.list
    sudo apt update -y
    sudo apt install -y postgresql-client-xx <-- UPDATE THIS VALUE WITH THE VERSION YOUR SERVER IS RUNNING AND REMOVE EVERYTHING ON THIS LINE AFTER xx
    
    
Usage:
    python dump_postgres_db.py --db-host <host> --db-port <port> --db-username <username> --db-password <password> --db-database <database> --backup-file <file>

Example:
    python dump_postgres_db.py --db-host localhost --db-port 5432 --db-username postgres --db-password password --db-database my_database --backup-file backup.sql
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Dump a PostgreSQL database to a SQL file"
    )
    ...
    parser.add_argument(
        "--config-file",
        default="postgres_dump_config.json",
...
def check_pg_dump_installed() -> bool:
    """Verify pg_dump installation."""
    log.info("Checking pg_dump installation")

    ## Test if pg_dump is installed
    if path := shutil.which("pg_dump"):
...
def save_default_config_json():
    ...
    with open("postgres_dump_config.json", "w") as f:
...
class DatabaseBackupController:
    """Controller class for PostgreSQL database backup operations.
    ...
    def _build_pg_dump_command(self) -> str:
        """Construct the pg_dump command."""
        return (
            f"pg_dump -h {self.db_settings.host} "
            f"-p {self.db_settings.port} "
            f"-U {self.db_settings.username} "
            f"-d {self.db_settings.database} "
            f"-f {self.db_settings.backup_file}"
        )
    ...
    def run_backup(self) -> None:
        ...
        if not check_pg_dump_installed():
            raise RuntimeError("pg_dump dependency not met")
        ...
        try:
            ...
            self.logger.error(f"({type(exc)}) Error running pg_dump command: {exc}")
            raise
        finally:
            if self._connection:
...
def main(
    ...
    try:
        ...
        log.error(f"Error dumping PostgreSQL database. Details: {exc}")
        exit(1)
...
if __name__ == "__main__":
    try:
        ...
        log.error(f"Error running PostgreSQL backup script. Details: {exc}")
        exit(1)
